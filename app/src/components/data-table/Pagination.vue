...
<template>
  <div class="p-3 flex flex-row items-center justify-between">
    <div v-if="totalRecords > 0">
      <select class="cursor-pointer w-14 text-sm text-gray-700 bg-gray-50 border border-gray-300 rounded py-1.5 px-2"
    ...
    <div v-else class="text-sm text-gray-700  py-1.5 px-2">
    ...
    <ul class="inline-flex items-stretch -space-x-px">
      ...
      <li v-for="page in visiblePages" :key="page" :class="{ active: page === currentPage }">
        <a @click="changePage(page)"
          class="flex cursor-pointer items-center justify-center text-sm py-2 px-3 leading-tight text-gray-500 border border-gray-300"
          :class="[page === currentPage ? 'bg-blue-100' : 'bg-white hover:bg-gray-100 hover:text-gray-700']">
          {{ page }}
        </a>
      </li>
      <li v-if="totalPages == 0" class="active">
        <span
          class="flex items-center justify-center text-sm py-2 px-3.5 leading-tight text-gray-500 border border-gray-300 bg-white ">
          0
        </span>
      </li>
      <li>
        <button @click="changePage(currentPage + 1)"
          :class="[(totalPages === 0 || currentPage === totalPages) ? 'border-gray-300 bg-gray-100' : 'bg-customBackgroundButtonDisabled']"
          :disabled="(totalPages === 0 || currentPage === totalPages)"
          class="flex items-center justify-center h-full py-1.5 px-3 leading-tight rounded-r border">
          <span class="sr-only">Next</span>
          <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg">
            <path :class="[(totalPages === 0 || currentPage === totalPages) ? 'fill-gray-400' : 'fill-white']"
